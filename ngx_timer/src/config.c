...
static const char* ip = "172.16.140.244";
static unsigned short port = 18600;
static unsigned int connection_pool_n = 20000; //optional, default 1
...
load_test_data()
{
	...
	uint32_t n_net;
	char buf[4];
	LOG_WARN("%s",p_config->test_data_file);
	...
	if (totoal_len <= 0 || totoal_len > 10000) { //array 1~1000
		LOG_WARN("totoal_len beyond 1~1000 %d",totoal_len);
		fclose(fp);
		return -1;
	}
